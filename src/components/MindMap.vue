...
export default class MindMap extends Vue {
  ...
  async makeCenter() { // 居中
    await d3.transition().end().then(() => {
      console.log(this.zoom.translateTo, 'zoom')
  ...
  appendNode(enter: d3.Selection<d3.EnterElement, FlexNode, Element, FlexNode>) {
    ...
    const colorArr = ['#606060', '#33a02c', '#21e014', '#f5cb00', '#1f78b4', '#a6cee3', '#bee29f', '#fb9a99', '#e8c7c6', '#65a52c']
    const sizeArr = ['20px', '19px', '18px', '17px', '16px', '15px', '14px', '13px']
    const foreign = gNode.append('foreignObject').attr('x', foreignX).attr('y', foreignY)
    console.log('aaaaa')
    const foreignDiv = foreign.append('xhtml:div').attr('contenteditable', false).style('color', (d: FlexNode) => d.data.color).style('color', (t: FlexNode) => {
      var dd = t.data.id.split('-')
      var ddd = (dd.length) % 10
      var color = colorArr[ddd]
      return color
    }).style('font-size', (t: FlexNode) => {
      var dd = t.data.id.split('-')
      var size = dd.length > 8 ? sizeArr[7] : sizeArr[dd.length - 1]
      return size
  ...
  draw() { // 生成svg
    ...
    const d = [this.root]
    console.log(mindmapG.selectAll('foreignObject > div'), 'asdfafd')
